...
def getSVSize(string_size):
  very_small_criterion = max(2, int(0.000001*string_size))
  small_criterion = max(2, int(0.00001*string_size))
  medium_criterion = max(2, int(0.001*string_size))
  medium_large_criterion = max(2, int(0.1*string_size))
  large_criterion = max(2, int(0.5*string_size))
  if(string_size < 15):
    return string_size
  if(string_size < 1000):
    small_size = 10 #minimal size of a structural variant
  elif(string_size >= 1000 and string_size < 1e6):
    small_size = 40 
  else: 
    small_size = 60
  maximal_size = int(0.9*string_size) 
  return_val0 = np.random.negative_binomial(1, 1/(very_small_criterion))
  return_val1 = np.random.negative_binomial(1, 1/(small_criterion))
  return_val2 = np.random.negative_binomial(1, 1/(medium_criterion))
  return_val3 = np.random.negative_binomial(1, 1/(medium_large_criterion))
  return_val4 = np.random.negative_binomial(1, 1/(large_criterion))
  zed = random.random()
  if(zed < 0.2): 
      o_return_val = return_val0
  elif(zed >= 0.2 and zed < 0.75):
      o_return_val = return_val1
  elif(zed >= 0.75 and zed < 0.95):
      o_return_val = return_val2
  elif(zed >= 0.95 and zed < 0.995):
      o_return_val = return_val3
  else: 
      o_return_val = return_val4
  if (o_return_val < small_size): 
      return small_size
  elif(o_return_val > maximal_size): 
      return maximal_size
  else: 
      return int(o_return_val)
...
def generateResults(storage_dir, dataid, num_samples_list, num_tumors_list, read_len_list, error_rate_list, frag_len_list, alpha_list, paired_list, WES_list, num_single_cell_list, coverage_list, liquid_biopsy_list, ctdna_frac_list, use_leaf_only, clone_list, pop_list, batch_size, subblock_size, LSH_hash, kmer_len,
    ...
    for tum in range(num_tumors):
        ...
        for sample in range(num_samples):
            ...
            num_single_cells = int(num_single_cell_list)
            #read_length_index = random.randint(0, len(read_len_list)-1)
            read_len = read_len_list
            frag_len = frag_len_list
            paired = paired_list
            WES = WES_list
    ...
    liquid_biopsy = liquid_biopsy_list
    if(liquid_biopsy):
        lb_dir = base_working_dir + 'liquid_biopsy/'
        makedir(lb_dir)
        clear_dir(lb_dir)
        lb_alpha = alpha_list
        lb_coverage = coverage_list
...
batch_size = 1
subblock_size = 1
LSH_hash = 0 #If false, doesn't explore alternate reads for WES and uses breakpoints in EXON_FILE
